...
class FEEDSYNC_TABLES {
	...
	public function create_listing_meta_table() {
                ...
                $check_exists = "SHOW TABLES LIKE 'listing_meta'";
                $results = fsdb()->get_results($check_exists);

                if( !empty( $results ) ) {
                        
                        $sql = "ALTER TABLE `listing_meta` RENAME TO `".fsdb()->listing_meta."`";
                        $rename_result = fsdb()->query($sql);
