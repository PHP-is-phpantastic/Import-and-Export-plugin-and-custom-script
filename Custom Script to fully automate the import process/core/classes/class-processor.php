...
class FEEDSYNC_PROCESSOR {
    ...
    */
     ...
     * @since 3.5.9 Fix: Correctly parse agent names with apostrophes in their name.
     */
    function process_agents( $item ) {
        ...
        if(!empty($listing_agents)) {
            foreach($listing_agents as $listing_agent) {
                ...
                $agent_exists = $this->db->get_row( $this->db->prepare("SELECT * FROM ".fsdb()->agent." where name = '%s'", stripslashes( $data_agent['name'] ) ) );
    ...
    **/
    ...
    function process_image($item = null) {
        ...
        $pdfs = $this->xpath->query('//document[@file]');

        if(!empty($pdfs)) {
            foreach ($pdfs as $k=>$pdf) {
                $pdf_name = trim($pdf->getAttribute('file'));
                if(!empty($pdf_name)) {
                    $pdf_name = basename($pdf_name);
                    $pdf_path = $this->get_url('files').$pdf_name;
                    $pdfs->item($k)->setAttribute('url', $pdf_path);
                    $this->logger_log('Document Processed : '.$pdf_path);
                }
     ...
     * @return boolean
     */
    function update_listing($db_data, $update_first_date = false ){
        ...
        if( $update_first_date ) {
                $first_date = "firstdate       = '{$db_data['firstdate']}',";
        }
    ...
    function complete_log() {
        ...
        if( isset($this->log_id) && intval($this->log_id) > 0) {
            ...
            if( isset($item->tagName) && !is_null($item->tagName) ) {
                ...
                if( !empty($exists) ) {
                    ...
                    if(  strtotime($exists->mod_date) <= strtotime($db_data['mod_date']) ) {
                        ...
                        $this->update_listing($db_data, $update_first_date );
