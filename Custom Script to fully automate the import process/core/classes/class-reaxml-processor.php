...
class REAXML_PROCESSOR extends FEEDSYNC_PROCESSOR {
     ...
     * @since 3.5.8 Changes to support log files that were processed to the listings
     */
    function import(){
        ...
        foreach($this->elements->childNodes as $item) {
            ...
            if( isset($item->tagName) && !is_null($item->tagName) ) {
                ...
                if( !empty($exists) ) {
                    ...
                    // check if listingAgent is missing in current data but is there in existing data
                    if ( !$this->has_node($item,'listingAgent') ) {
                        ...
                        if( $this->has_node( $existing_xml,'listingAgent' ) ) {

                                $this->logger_log('No agents found in the listing XML');

                                /** fetch all agents node from xml in DB */
                                $listingagents = $this->get_nodes($existing_xml,'listingAgent');
                                
                                if( $listingagents->length > 0 ) {

                                        $this->logger_log('Agents found in existing xml in DB');

                                        foreach( $listingagents as $listingagent ) {
                                                
                                                $backup_listingagent_node  = $this->xmlFile->importNode($listingagent, true);
                                                $item->appendChild($backup_listingagent_node);
                                        }
                
                                        
                                        $db_data['xml']             = $this->xmlFile->saveXML( $item);
                                        $this->logger_log('Copied agents from existing xml in DB');
                                }
                    ...
                    if(  strtotime($exists->mod_date) <= strtotime($db_data['mod_date']) ) {
                        ...
                        if ( !$this->has_node($item,'firstDate')) {
                            ...
                            $this->logger_log('First Date not found, added firstDate:'.$exists->firstdate);
                                ...
                                // if first date is already there. Check for status change
                                if( 'current' == $db_data['status'] && in_array( $exists->status, ['sold', 'leased', 'withdrawn' ] ) ) {
                                        // recreate firstdate
                                        $this->logger_log( 'First Date Reset to current Mod Date : '. $db_data['mod_date'] );
                                        $db_data['firstdate'] = $this->convert_time_to_timezone( $db_data['mod_date'], get_option('feedsync_timezone') );
                                        $item = $this->set_node_value( $item,'firstDate', $db_data['firstdate'] );
                                        $update_first_date = true;
                                }
                        ...
                        //print_exit($db_data);
                        $insert_id                              = $this->update_listing($db_data, $update_first_date);
                        $listing_id                             = get_listing_id_by_fs_unique_id( $db_data['feedsync_unique_id'] );
                        $listing_file_array                     = (array) get_post_meta( $listing_id, 'file_processed', true );
                        $listing_file_array                     = array_filter( $listing_file_array );
                        $listing_file_array[ microtime() ]           = basename($this->path);

                        if( $insert_id ) {
                                update_post_meta( $listing_id ,'file_processed', $listing_file_array );
                        }
                    ...
                    $insert_id                          = $this->insert_listing($db_data);
                    $listing_id                         = get_listing_id_by_fs_unique_id( $db_data['feedsync_unique_id'] );
                    $listing_file_array                 = (array) get_post_meta( $listing_id, 'file_processed', true );
                    $listing_file_array                 = array_filter( $listing_file_array );
                    $listing_file_array[ microtime() ]       = basename($this->path);
